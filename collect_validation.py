...
folder = os.path.join("validation", letter)
os.makedirs(folder, exist_ok=True)

existing = len([f for f in os.listdir(folder) if f.endswith('.jpg')])
count = existing

print(f"\nCollecting VALIDATION photos for: {letter}")
print(f"Existing photos: {count}")
print(f"Goal: 3-4 photos")
...
while True:
    ...
    cv2.putText(frame, f"VALIDATION - Letter: {letter}", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 165, 0), 2)
    cv2.putText(frame, f"Photos: {count}/4", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 165, 0), 2)
    ...
    cv2.imshow("Validation Collection", frame)
...
print(f"\nDone! {letter} has {count} validation photos")